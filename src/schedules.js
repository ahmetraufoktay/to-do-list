...
today.addEventListener("click", () => {
  resetSection();
  header.innerText = "Tasks Due Today";
...
tomorrow.addEventListener("click", () => {
  resetSection();
  header.innerText = "Tasks Due Tomorrow";
...
thisweek.addEventListener("click", () => {
  resetSection();
  header.innerText = "Tasks Until This Week";
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    for (const task of taskList) {
      ...
      if (
        (compareAsc(weekDate, fixedDate) == 1 &&
          compareAsc(fixedDate, Date()) == 1) ||
        isToday(fixedDate)
      ) {
        const taskElement = new TaskElement(
          task["name"],
